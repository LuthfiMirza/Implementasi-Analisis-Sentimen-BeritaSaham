...
## Fase EW — Audit SELF_RADAR_V1 CUAN 2026-09-03 entry price ambiguity

Status: DICATAT. User menemukan angka CUAN `Rp925` di rekap BUY kemarin tidak sesuai praktik entry manual ujung bursa (`2026-09-03 15:40-15:45`, kisaran `Rp905-910`). Audit 2026-09-04 11:06 WIB: tabel `stock_prices` untuk CUAN hanya punya `interval_type=1d`, tanpa data intraday/jam. Baris harian `2026-09-03` CUAN: open `915`, high `940`, low `900`, close `940`, volume `260466000`. Live quote saat audit 2026-09-04 sekitar 11:06 WIB: last `935`, high `940`, low `900`, source `yahoo_finance`. Kesimpulan: sistem belum menyimpan `first_seen_at`, `signal_price_at_seen`, atau harga entry aktual jam 15:40-15:45 untuk SELF_RADAR_V1; angka kartu radar adalah live quote saat render/fetch, bukan bukti harga fill entry. `research:detect-drawdown-bounce-signal` jalan 15:18 WIB tetapi bukan sumber SELF_RADAR_V1 CUAN dan CUAN tidak termasuk universe resmi command itu. Jangan pakai Rp925 sebagai entry journal CUAN 2026-09-03 kalau entry manual user Rp905-910. Perlu tabel/log audit SELF_RADAR_V1 sebelum mengubah logika: ticker, status, first_seen_at, last_seen_at, price_at_first_seen, latest_price, rsi14, ret_5d, dd_20d, source.

## Fase EX — SELF_RADAR_V1 all-stock scan comparison 2026-09-04

Status: DICATAT. Audit 2026-09-04 11:13 WIB menjalankan rule SELF_RADAR_V1 (`RSI14 >= 60`, `ret_5d >= 5%`, `dd_20d >= -5%`) ke semua saham aktif yang punya data `stock_prices` harian. Catatan penting: `price_date` DB tampil geser UTC; untuk tanggal BEI lokal, `2026-09-03` harus dibaca dari baris date lokal, bukan substring UTC mentah. Hasil: sinyal kemarin semua-universe `n=21`, jika entry pakai close harian 2026-09-03 dan dinilai ke 2026-09-04 11:13, performa `14 WIN / 1 DRAW / 6 LOSS`, rata-rata `+2.22%`. Subset 6 ticker yang user coba (`SINI,IATA,CUAN,ESSA,PACK,INET`) dengan entry close harian DB: `4 WIN / 1 DRAW / 1 LOSS`, rata-rata `+5.91%` — lebih baik daripada scan semua saham karena scan semua memasukkan loser `KOTA`, `KIJA`, `BWPT`, `ESSA`, `DMAS`, `AUTO`. CUAN khusus: entry DB lokal 2026-09-03 close `880`, bukan `925`; jika user masuk manual 15:40-15:45 di `905-910`, tetap WIN tapi return lebih rendah dari close DB. Kandidat sekarang 2026-09-04 semua-universe masih `n=21`; belum bisa dinilai profit sampai sesi berikutnya. Rekomendasi: jangan perluas SELF_RADAR_V1 ke semua saham tanpa ranking/filter likuiditas dan log `first_seen_at`; subset pilihan user kemarin justru lebih baik dari all-stock scan pada tes 1 hari ini.
