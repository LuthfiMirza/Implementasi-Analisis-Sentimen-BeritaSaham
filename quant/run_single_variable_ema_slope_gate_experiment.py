"""Run the locked single-variable Layer 2 EMA slope gate experiment across all approved variants."""
...
SUMMARY_OUTPUT = "single_variable_experiment_results_v3.json"
REPORT_OUTPUT = "single_variable_experiment_results_v3.txt"

LOCKED_VARIANTS = ["none", "ema20_today_gt_ema20_prev_day"]
...
def run_single_variable_ema_slope_gate_experiment(
    *,
    output_dir: Path,
    ...
    experiment_run_root = output_dir / "single_variable_experiment_runs_v3"
    experiment_run_root.mkdir(parents=True, exist_ok=True)

    results_by_variant: List[Dict[str, object]] = []
    for variant_id in LOCKED_VARIANTS:
        variant_dir = experiment_run_root / str(variant_id)
        run_result = run_walk_forward_validation(
            ...
            layer2_confirmation_days=0,
            layer2_short_term_ema_slope_gate=str(variant_id),
        )
        ...
        result_row["assessment_note"] = (
            ...
            else "Fails at least one locked minimum criterion; no escape clause, slope-definition expansion, or second variable allowed."
        )
    ...
    if passing_variants:
        ...
        winner_variant: Optional[str] = str(winner["variant_id"])
        ...
        hypothesis_result_reason = (
            f"At least one approved EMA-slope gate variant passed the locked criteria; variant {winner_variant} "
            "is the most defensible passing variant after comparing all approved values."
        )
    else:
        winner_variant = None
        ...
        hypothesis_result_reason = (
            "None of the approved EMA-slope gate variants passed the locked minimum criteria, so the hypothesis "
            "is rejected without expanding the slope definition, loosening pass criteria, or adding a second variable."
        )

    summary_payload = {
        "artifact": "single_variable_experiment_results_v3",
        ...
        "tested_variants": [str(value) for value in LOCKED_VARIANTS],
        ...
        "next_action": "review_single_variable_experiment_result_v3_against_locked_criteria_before_any_further_strategy_change",
    }
    ...
    report_lines = [
        "Single Variable Layer 2 EMA Slope Gate Experiment Results",
        "=========================================================",
        "",
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the locked single-variable Layer 2 EMA slope gate experiment.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="output/one_small_controlled_strategy_hypothesis_v3.json",
        help="Locked hypothesis v3 artifact path.",
    )
    parser.add_argument(
        "--spec-reference",
        default="output/single_variable_experiment_spec_v3.json",
        help="Locked experiment v3 spec artifact path.",
    )
    return parser
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    run_single_variable_ema_slope_gate_experiment(
