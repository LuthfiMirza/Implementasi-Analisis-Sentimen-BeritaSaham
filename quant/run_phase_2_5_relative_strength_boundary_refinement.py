"""Boundary-cut refinement pass for the Layer 2 relative strength candidate."""
...
def _build_boundary_variant_registry() -> List[BoundaryVariant]:
    return [
        BoundaryVariant(
            variant_id="top_22_5pct_return_20d",
            label="Top 22.5% by return_20d",
...
def _build_boundary_cut_summary(slice_variant_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(rows).reindex(columns=BOUNDARY_SUMMARY_COLUMNS)


def _select_boundary_cut(boundary_df: pd.DataFrame) -> Dict[str, object]:
    if boundary_df.empty:
        return {
            "status": "no_candidate",
            "selected_variant_id": None,
            "selected_variant_label": None,
            "reason": "Boundary refinement did not produce any candidate rows.",
        }

    eligible = boundary_df[
        boundary_df["full_period_tickers_with_coverage_collapse"].eq(0)
        & boundary_df["full_period_sample_adequacy_risk"].eq("low")
        & boundary_df["full_period_avg_delta_average_return"].gt(0)
    ].copy()
    if eligible.empty:
        eligible = boundary_df.copy()
    ...
    return {
        ...
        "reason": (
            "Selected the narrow boundary cut with the strongest slice robustness, then used "
            "full-period quality delta and retained sample as tie-breakers."
        ),
    }


def _freeze_decision(boundary_df: pd.DataFrame, candidate_variant_id: str) -> Dict[str, object]:
    candidate = boundary_df[boundary_df["variant_id"] == candidate_variant_id]
    if candidate.empty:
        return {
            "freeze_layer_2_candidate": False,
            "candidate_variant_id": candidate_variant_id,
            "reason": "Selected boundary cut is missing from the Phase 2.5 summary.",
            "remaining_blocker": "selected_boundary_cut_missing_from_phase_2_5_summary",
        }
    ...
    robust_enough = bool(
        ...
        and float(row["candidate_avg_delta_average_return_range"]) <= 0.75
    )

    if robust_enough:
        blocker = None
        reason = (
            "Selected boundary cut is positive on the full period, wins a clear majority of robustness slices, "
            "and keeps slice-to-slice quality dispersion within a freezeable range."
        )
    else:
        if win_count < max(4, slice_count - 2):
            blocker = "no_boundary_cut_yet_breaks_slice_concentration_blocker"
        elif float(row["candidate_avg_delta_average_return_range"]) > 0.75:
            blocker = "best_boundary_cut_still_has_too_wide_slice_quality_dispersion"
        elif int(row["candidate_negative_return_slice_count"]) > 1:
            blocker = "best_boundary_cut_still_turns_negative_in_too_many_slices"
        else:
            blocker = "best_boundary_cut_still_not_decisive_enough_for_final_freeze"
        reason = "Selected boundary cut still falls short of the freeze threshold for Layer 2."
    ...
    return {
        ...
        "boundary_diagnostics": row.to_dict(),
    }


def run_phase_2_5_relative_strength_boundary_refinement(
    *,
    stock_indicator_master_file: Path,
    ihsg_indicator_master_file: Path,
    phase_2_4_summary_file: Path,
    output_dir: Path,
    hold_period: int = 5,
    ...
    variants = _build_boundary_variant_registry()
    ...
    boundary_summary_df = _build_boundary_cut_summary(slice_variant_df)
    selection_decision = _select_boundary_cut(boundary_summary_df)
    freeze_decision = _freeze_decision(
        boundary_summary_df,
        candidate_variant_id=str(selection_decision.get("selected_variant_id")),
    )
    phase_2_4_summary = json.loads(phase_2_4_summary_file.read_text(encoding="utf-8"))

    output_dir.mkdir(parents=True, exist_ok=True)
    per_slice_path = output_dir / "phase_2_5_relative_strength_boundary_refinement_per_slice.csv"
    per_cut_path = output_dir / "phase_2_5_relative_strength_boundary_refinement_per_cut.csv"
    summary_path = output_dir / "phase_2_5_relative_strength_boundary_refinement_summary.json"
    report_path = output_dir / "phase_2_5_relative_strength_boundary_refinement_report.txt"

    slice_variant_df.to_csv(per_slice_path, index=False)
    boundary_summary_df.to_csv(per_cut_path, index=False)

    summary_payload = {
        "phase": "phase_2_5_relative_strength_boundary_refinement",
        "status": "completed",
        "generated_at": _now_iso(),
        "candidate_under_refinement": "top_25pct_return_20d",
        "source_files": {
            "stock_indicator_master_file": str(stock_indicator_master_file),
            "ihsg_indicator_master_file": str(ihsg_indicator_master_file),
            "phase_2_4_summary_file": str(phase_2_4_summary_file),
        ...
        "prior_status_context": {
            "phase_2_4_candidate_variant_id": phase_2_4_summary.get("freeze_decision", {}).get("candidate_variant_id"),
            "phase_2_4_freeze_layer_2_candidate": phase_2_4_summary.get("freeze_decision", {}).get("freeze_layer_2_candidate"),
            "phase_2_4_remaining_blocker": phase_2_4_summary.get("freeze_decision", {}).get("remaining_blocker"),
        },
        "cuts_tested": [
            {
                "variant_id": variant.variant_id,
        ...
        "boundary_cut_summary": boundary_summary_df.to_dict(orient="records"),
    ...
    report_lines = [
        "Phase 2.5 - Boundary Cut Refinement",
        "===================================",
        "",
        "Cuts tested:",
    ]
    ...
    report_lines.extend(
        [
            "",
            "Decision:",
            f"- selected_boundary_cut = {selection_decision['selected_variant_id']}",
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Phase 2.5 boundary-cut refinement for Layer 2.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--phase-2-4-summary-file",
        default="output/phase_2_4_relative_strength_robustness_summary.json",
        help="Phase 2.4 robustness summary JSON used as source of truth.",
    )
    parser.add_argument("--output-dir", default="output", help="Directory for Phase 2.5 artifacts.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    run_phase_2_5_relative_strength_boundary_refinement(
        ...
        phase_2_4_summary_file=Path(args.phase_2_4_summary_file),
