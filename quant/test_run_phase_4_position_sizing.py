...
from quant.run_phase_4_position_sizing import _apply_position_sizing, run_phase_4_position_sizing
...
class RunPhase4PositionSizingTestCase(unittest.TestCase):
    def test_apply_position_sizing_caps_position(self) -> None:
        ...
        sized, metrics = _apply_position_sizing(trades_df, variant)
        self.assertAlmostEqual(float(sized.iloc[0]["position_size_pct"]), 10.0, places=4)
        self.assertAlmostEqual(float(metrics["cap_hit_rate_pct"]), 100.0, places=4)
    ...
    def test_runner_exports_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_phase_4_position_sizing(
            ...
            self.assertTrue((output_dir / "phase_4_position_sizing_summary.json").exists())
            self.assertTrue((output_dir / "phase_4_position_sizing_report.txt").exists())
            self.assertTrue((output_dir / "phase_4_position_sizing_per_ticker.csv").exists())
            self.assertTrue((output_dir / "phase_4_position_sizing_trades.csv").exists())
