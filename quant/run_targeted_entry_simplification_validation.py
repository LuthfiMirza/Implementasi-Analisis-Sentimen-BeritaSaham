"""Evaluate the targeted entry simplification candidate against the current control stack."""
...
SUMMARY_OUTPUT = "targeted_entry_simplification_validation_summary.json"
REPORT_OUTPUT = "targeted_entry_simplification_validation_report.txt"
...
def _build_variant_registry() -> List[EntryVariant]:
    return [
        EntryVariant(
            variant_id="control_layer1_return20d_positive_and_close_above_ema50",
            label="Control: Layer 1 + return_20d > 0 + close > ema50 + ATR trailing stop + sizing",
            entry_policy="control_dual_gate",
        ...
        EntryVariant(
            variant_id="candidate_layer1_return20d_positive_only",
            label="Candidate: Layer 1 + return_20d > 0 only + ATR trailing stop + sizing",
            entry_policy="momentum_positive_only",
            layer_3_active=False,
            position_sizing_active=True,
            risk_per_trade_pct=2.0,
            max_position_pct=10.0,
...
def _apply_entry_policy(frame: pd.DataFrame, variant: EntryVariant) -> pd.DataFrame:
    ...
    if variant.entry_policy == "control_dual_gate":
    ...
    elif variant.entry_policy == "momentum_positive_only":
        working["active_selected"] = (
            working["market_regime_bullish"].astype(bool)
            & working["alt_data_ready"].astype(bool)
            & working["alt_momentum_positive"].astype(bool)
...
def _compare_candidate_vs_control(
    ...
    control_full = full_period_results["control_layer1_return20d_positive_and_close_above_ema50"]
    candidate_full = full_period_results["candidate_layer1_return20d_positive_only"]
    control_wf = walk_forward_results["control_layer1_return20d_positive_and_close_above_ema50"]
    candidate_wf = walk_forward_results["candidate_layer1_return20d_positive_only"]
    ...
    drop_decision = (
        ...
        if drop_close_above_ema50
        else "not_yet_keep_under_review"
    )
    reason = (
        "Candidate baru menunjukkan ketahanan OOS/holdout yang lebih baik secara mayoritas metrik utama, sehingga `close > ema50` layak dibuang dari stack inti secara provisional."
        if drop_close_above_ema50
        else "Candidate baru belum cukup tegas mengalahkan control di OOS/holdout, sehingga `close > ema50` belum resmi dibuang dari stack inti."
    )
    return {
        ...
        "close_above_ema50_drop_decision": drop_decision,
        "reason": reason,
    }


def run_targeted_entry_simplification_validation(
    *,
    ...
    summary_payload = {
        "phase": "targeted_entry_simplification_validation",
        "status": "completed",
        ...
        "source_files": {
            ...
            "entry_simplification_plan": "output/targeted_entry_simplification_plan.json",
            "walk_forward_closeout": "output/walk_forward_validation_closeout.json",
    ...
    report_lines = [
        "Targeted Entry Simplification Validation",
        "=======================================",
        "",
        ...
        f"- close_above_ema50_drop_decision = {comparison_decision['close_above_ema50_drop_decision']}",
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate the targeted entry simplification candidate versus the current control.")
    parser.add_argument(
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    run_targeted_entry_simplification_validation(
