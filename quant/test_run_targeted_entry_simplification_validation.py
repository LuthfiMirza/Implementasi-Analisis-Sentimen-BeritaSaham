...
from quant.run_targeted_entry_simplification_validation import (
    _apply_entry_policy,
...
def _build_stock_indicator_master_frame(
    *,
    ...
    ema50_series = close_series - 1.0 if close_above_ema50 else close_series + 1.0
    return pd.DataFrame(
        {
            "date": dates,
            "ticker": ticker,
...
class RunTargetedEntrySimplificationValidationTestCase(unittest.TestCase):
        ...
        frame = pd.DataFrame(
            {
                ...
                "alt_momentum_positive": [True, True, True, True],
                "alt_close_above_ema50": [True, False, True, False],
                "phase_1_signal_layer1": [True, True, True, True],
            }
        )
        variants = {variant.variant_id: variant for variant in _build_variant_registry()}
        control = _apply_entry_policy(frame, variants["control_layer1_return20d_positive_and_close_above_ema50"])
        candidate = _apply_entry_policy(frame, variants["candidate_layer1_return20d_positive_only"])

        self.assertEqual(2, int(control["entry_signal"].sum()))
        self.assertEqual(4, int(candidate["entry_signal"].sum()))

    def test_runner_exports_artifacts_and_candidate_is_broader_than_control(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_targeted_entry_simplification_validation(
            ...
            self.assertTrue((output_dir / "targeted_entry_simplification_validation_summary.json").exists())
            self.assertTrue((output_dir / "targeted_entry_simplification_validation_report.txt").exists())
            variants = {item["variant_id"]: item for item in result["summary"]["variants_compared"]}
            self.assertIn("control_layer1_return20d_positive_and_close_above_ema50", variants)
            self.assertIn("candidate_layer1_return20d_positive_only", variants)
