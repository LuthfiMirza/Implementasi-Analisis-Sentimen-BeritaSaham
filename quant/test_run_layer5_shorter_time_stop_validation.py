...
from quant.run_layer5_shorter_time_stop_validation import (
    _build_variant_registry,
    run_layer5_shorter_time_stop_validation,
)
...
class RunLayer5ShorterTimeStopValidationTestCase(unittest.TestCase):
    def test_variant_registry_contains_expected_time_stops(self) -> None:
        variants = {variant.variant_id: variant for variant in _build_variant_registry()}
        self.assertEqual(15, variants["control_atr25_time_stop_15d"].time_stop_days)
        self.assertEqual(12, variants["candidate_atr25_time_stop_12d"].time_stop_days)
        self.assertEqual(10, variants["candidate_atr25_time_stop_10d"].time_stop_days)
    ...
    def test_runner_exports_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue((output_dir / "layer5_shorter_time_stop_validation_summary.json").exists())
            self.assertTrue((output_dir / "layer5_shorter_time_stop_validation_report.txt").exists())
            self.assertTrue((output_dir / "layer5_shorter_time_stop_validation_closeout.json").exists())
            self.assertTrue((output_dir / "layer5_shorter_time_stop_validation_closeout.txt").exists())
