"""Run the locked single-variable Layer 2 threshold experiment across all approved values."""
...
SUMMARY_OUTPUT = "single_variable_experiment_results.json"
REPORT_OUTPUT = "single_variable_experiment_results.txt"

LOCKED_THRESHOLDS = [0.0, 0.01, 0.02, 0.03]
...
def run_single_variable_layer2_threshold_experiment(
    *,
    output_dir: Path,
    ...
    experiment_run_root = output_dir / "single_variable_experiment_runs"
    ...
    for threshold in LOCKED_THRESHOLDS:
        variant_dir = experiment_run_root / f"threshold_{threshold:.2f}"
        run_result = run_walk_forward_validation(
            ...
            layer2_momentum_floor_on_return_20d=float(threshold),
        )
        ...
        result_row["assessment_note"] = (
            ...
            else "Fails at least one locked minimum criterion; no escape clause or range expansion allowed."
        )
    ...
    if passing_variants:
        ...
        winner_variant: Optional[float] = float(winner["threshold_value"])
        ...
        hypothesis_result_reason = (
            f"At least one approved threshold passed the locked criteria; threshold {winner_variant:.2f} "
            "is the most defensible passing variant after comparing all four approved values."
        )
    else:
        winner_variant = None
        ...
        hypothesis_result_reason = (
            "None of the four approved threshold values passed the locked minimum criteria, so the hypothesis "
            "is rejected without expanding the range, loosening pass criteria, or adding a second variable."
        )

    summary_payload = {
        "artifact": "single_variable_experiment_results",
        ...
        "tested_variants": [float(value) for value in LOCKED_THRESHOLDS],
        ...
        "next_action": (
            "review_single_variable_experiment_result_against_locked_criteria_before_any_further_strategy_change"
        ),
    }
    ...
    report_lines = [
        "Single Variable Layer 2 Threshold Experiment Results",
        "====================================================",
        "",
    ...
    for row in results_by_variant:
        report_lines.extend(
            [
                f"Variant threshold {float(row['threshold_value']):.2f}",
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the locked single-variable Layer 2 threshold experiment.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="output/one_small_controlled_strategy_hypothesis.json",
        help="Locked hypothesis artifact path.",
    )
    parser.add_argument(
        "--spec-reference",
        default="output/single_variable_experiment_spec.json",
        help="Locked experiment spec artifact path.",
    )
    return parser
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    run_single_variable_layer2_threshold_experiment(
