<x-app-layout>
    <div class="glass-card p-6 max-w-2xl">
        <h1 class="text-2xl font-bold mb-4">Edit Trade Journal</h1>
        <form action="{{ route('trade-journal.update', $trade) }}" method="POST" class="space-y-4">
            @csrf
            @method('PATCH')
            @include('trade-journal.partials.form', ['trade' => $trade])
