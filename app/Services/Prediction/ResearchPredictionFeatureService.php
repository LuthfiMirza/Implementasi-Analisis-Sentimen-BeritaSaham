...
class ResearchPredictionFeatureService
{
    ...
    public function buildForDate(
        ...
        $regimeBullish = $this->ihsgRegimeMap()->get($referencePoint->toDateString());
        ...
        return [
            'prediction_feature_version' => 'technical_prediction_research_v1',
    ...
    public function seriesForStock(Stock $stock): Collection
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            $result->put($row['date'], [
                ...
                'return_5d' => $return5,
                'return_20d' => $return20,
                'atr14_pct' => ($atr14 !== null && $close != 0.0) ? round($atr14 / $close, 6) : null,
                'volume_ratio_20d' => ($volumeMa20 !== null && $volumeMa20 != 0.0) ? round($volume / $volumeMa20, 6) : null,
    ...
    protected function ihsgRegimeMap(): Collection
    {
        ...
        if (! is_file($this->ihsgCsvPath)) {
            return $this->ihsgRegimeCache = collect();
        }
        ...
        foreach ($rows as $index => $row) {
            ...
            $map->put(
                $row['date'],
                $index >= 199 ? ($ema50 > $ema200) : null,
            );
        }

        return $this->ihsgRegimeCache = $map;
    }
    ...
    protected function sentimentWindowStats(
        ...
        foreach ($articles as $article) {
            ...
            if ($published->lt($periodStart) || $published->gt($referencePoint)) {
                continue;
            }
